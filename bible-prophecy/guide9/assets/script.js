(() => {
  ...
  const storageKey = "tjm-bible-prophecy-guide-9-progress";
  ...
  const storage = {
    ...
    set(key, value) {
      try { window.localStorage.setItem(key, value); } catch { /* Progress saving is optional. */ }
    },
    remove(key) {
      try { window.localStorage.removeItem(key); } catch { /* Progress saving is optional. */ }
    }
  };
  ...
  const saved = Number.parseInt(storage.get(storageKey) || "1", 10);
  ...
  const updateUI = ({ announce = false, scroll = false } = {}) => {
    ...
    storage.set(storageKey, String(current));
    stopSpeech();
  ...
  listenButton?.addEventListener("click", () => {
    ...
    const text = activePanel?.innerText.replace(/↗/g, "").replace(/→/g, "").replace(/\s+/g, " ").trim();
    if (!text) return;
