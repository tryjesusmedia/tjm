...
const PRINCIPLES_PLAN_ID = CONFIG?.notesPlanId || CONFIG?.planId;

if (!CONFIG) {
  console.warn("Folder Mind Map: reading-plan configuration was not found.");
} else {
  const SENTINEL_READING_ID = PRINCIPLES_PLAN_ID === "bible-conflict-ages-v1" ? "coa-000" : "chron-000-00";
    ...
    function read() {
      try {
        ...
        if (current && typeof current === "object") {
          return { ...defaultLayout(), ...current, mapOpen: false };
        }
  ...
  function useBridgeRows(bridge) {
    ...
    useEffect(() => {
      const update = (event) => {
        if (event.detail?.planId === PRINCIPLES_PLAN_ID) setRows(activeRows(event.detail.rows || []));
      };
      ...
      <article className=${`tjm-fm-node tjm-fm-principle ${data.expanded ? "is-expanded" : ""} ${selected ? "is-selected" : ""}`}
        ...
        <div className="tjm-fm-principle-row">
          ...
          <button type="button" className="tjm-fm-principle-preview nodrag nopan"
            ...
            <span className="tjm-fm-principle-summary-text">${firstWords(principle.body, 11)}</span>
          </button>
        ...
        ${data.expanded && html`
          <div className="tjm-fm-principle-body nodrag nopan nowheel" tabIndex=${0}
            aria-label=${`Principle ${principle.principle_number} details`}>
            <p className="tjm-fm-principle-body-text">${principle.body}</p>
      ...
      <article className=${`tjm-fm-list-principle ${expanded ? "is-expanded" : ""}`} data-fm-list-principle-id=${principle.id}>
        <div className="tjm-fm-list-principle-row">
          ...
          <button type="button" className="tjm-fm-list-principle-open" aria-expanded=${String(expanded)}
            ...
            <strong>${principleName(principle)}</strong>
            <span className="tjm-fm-principle-summary-text">${firstWords(principle.body, 16)}</span>
        ...
        ${expanded && html`
          <div className="tjm-fm-list-principle-detail" tabIndex=${0}
            aria-label=${`Principle ${principle.principle_number} details`}>
            <p className="tjm-fm-principle-body-text">${principle.body}</p>
            ${(principle.cross_reference_numbers || []).length > 0 && html`
        ...
        ${!currentFolder && html`
          <section className="tjm-fm-list-section" aria-labelledby="tjm-fm-folders-heading">
            ...
            <div className="tjm-fm-folder-list">
              ...
              `)}
              ${allFolders.length === 0 && html`<p className="tjm-fm-list-empty">No folders yet. Use the menu to create one.</p>`}
            </div>
          </section>
        `}
  ...
  function ContextMenu({ menu, busy, onClose, onEdit, onDuplicate, onAddToFolder, onRemoveFromFolder, onDelete,
    ...
    const label = menu.kind === "principle" ? principleName(menu.item)
      : menu.kind === "folder" ? menu.item.title : "Principles Map";
    ...
    return html`
      <div className="tjm-fm-dialog-layer tjm-fm-menu-layer">
        <section className="tjm-fm-context-menu" role="dialog" aria-modal="true"
          aria-label=${menu.kind === 'folder' ? 'Folder menu' : menu.kind === 'principle' ? 'Principle menu' : 'Principles Map menu'}
          style=${style} ref=${ref}>
          ...
          <div role="menu">
            ...
            ` : html`
              <button type="button" role="menuitem" disabled=${busy} onClick=${() => onNewPrinciple(null)}><span aria-hidden="true">＋</span>New Principle</button>
              <button type="button" role="menuitem" disabled=${busy} onClick=${onNewFolder}><span aria-hidden="true">▰</span>New Folder</button>
              <button type="button" role="menuitem" disabled=${busy} onClick=${() => onFind(null)}><span aria-hidden="true">⌕</span>Find a Principle</button>
              <button type="button" role="menuitem" disabled=${busy} onClick=${onArrange}><span aria-hidden="true">▦</span>Arrange Automatically</button>
              <button type="button" role="menuitem" disabled=${busy} onClick=${onFit}><span aria-hidden="true">⌗</span>Fit All</button>
            `}
          </div>
  ...
  function FlowCanvas({ nodes, onNodesChange, mapId, viewport, onViewport, onInit, onNodeDrag, onNodeDragStop, onPaneClick, emptyMessage, compact }) {
    const containerRef = useRef(null);
    const flowRef = useRef(null);
    const [flowReady, setFlowReady] = useState(false);
    const [canvasSize, setCanvasSize] = useState(() => ({
      width: Math.max(1, globalThis.innerWidth || 1),
      height: Math.max(1, globalThis.innerHeight || 1),
    }));
    const [zoom, setZoom] = useState(() => clamp(Number(viewport?.zoom) || 0.78, 0.42, 1));

    useEffect(() => {
      const element = containerRef.current;
      if (!element) return undefined;
      const measure = () => setCanvasSize({
    ...
    const worldWidth = canvasSize.width / safeZoom;
    const topInset = compact ? 16 : 28;
    const bottommostNodeTop = Math.max(0, ...nodes.map((node) => Number(node.position?.y) || 0));
    const translateExtent = useMemo(() => [
      [0, 0],
      [worldWidth, Math.max(
        canvasSize.height / safeZoom,
        bottommostNodeTop + ((canvasSize.height - topInset) / safeZoom),
      )],
    ], [worldWidth, bottommostNodeTop, canvasSize.height, topInset, safeZoom]);
    const nodeWidth = compact ? 318 : 390;
    const horizontalPadding = compact ? 12 : 24;
    const contentRight = Math.max(nodeWidth, ...nodes.map((node) =>
      (Number(node.position?.x) || 0) + (node.type === "principle" && node.data?.expanded ? nodeWidth : (compact ? 270 : 310))));
    const expandedNodeTop = Number(nodes.find((node) => node.type === "principle" && node.data?.expanded)?.position?.y);
    const maxZoom = clamp(canvasSize.width / Math.max(canvasSize.width, contentRight + horizontalPadding), 0.1, 1);
    const minZoom = Math.min(0.42, maxZoom);
    const nodeExtent = useMemo(() => [
      [horizontalPadding, 16],
      [Math.max(horizontalPadding, worldWidth - nodeWidth - horizontalPadding), 1_000_000],
    ], [horizontalPadding, worldWidth, nodeWidth]);
    const initialViewport = viewport
      ? { ...viewport, x: 0, zoom: clamp(Number(viewport.zoom) || 0.78, minZoom, maxZoom) }
      : { x: 0, y: 0, zoom: Math.min(0.78, maxZoom) };

    useEffect(() => {
      const flow = flowRef.current;
      if (!flowReady || !flow) return;
      const current = flow.getViewport();
      const nextZoom = clamp(current.zoom, minZoom, maxZoom);
      const expandedScreenTop = Number.isFinite(expandedNodeTop) ? (expandedNodeTop * nextZoom) + current.y : topInset;
      const nextY = expandedScreenTop < topInset ? topInset - (expandedNodeTop * nextZoom) : current.y;
      if (Math.abs(current.x) > 0.5 || Math.abs(current.y - nextY) > 0.5 || Math.abs(current.zoom - nextZoom) > 0.001) {
        setZoom(nextZoom);
        flow.setViewport({ x: 0, y: nextY, zoom: nextZoom });
      }
    }, [flowReady, mapId, minZoom, maxZoom, canvasSize.width, contentRight, expandedNodeTop, topInset]);

    return html`
      <div className="tjm-fm-flow" ref=${containerRef} data-horizontal-pan="locked">
        <${ReactFlow}
          nodes=${nodes}
          edges=${[]}
          nodeTypes=${nodeTypes}
          ...
          onMove=${(_, nextViewport) => {
            if (Math.abs(nextViewport.zoom - zoom) > 0.001) setZoom(nextViewport.zoom);
          }}
          onMoveEnd=${(_, nextViewport) => onViewport(mapId, { ...nextViewport, x: 0 })}
          ...
          ${nodes.length > 8 && html`<${MiniMap} position="top-right" pannable=${false} zoomable=${true}
  ...
  function FolderMindMap({ bridge }) {
    ...
    const [layout, setLayout] = usePersistentLayout(PRINCIPLES_PLAN_ID, userId, bridge);
    ...
    const goToReading = useCallback((principle) => {
      clearOverlays();
      setExpandedId("");
      setOpenFolderId("");
      const nextHistory = { ...history.state };
      delete nextHistory.tjmMindMapOverlay;
      delete nextHistory.tjmMindMapPrinciple;
      delete nextHistory.tjmFolderMindMap;
      history.replaceState(nextHistory, "");
      mutateLayout((next) => {
        next.mapOpen = false;
        return next;
      });
      bridge.options.goToReadingById?.(principle.reading_id);
    }, [bridge, clearOverlays, mutateLayout]);
    ...
    const callbacks = useCallback(() => ({
      ...
      readingLabel,
    }), [togglePrinciple, findReference, goToReading, openMenu, readingLabel]);
    ...
    const buildMainNodes = useCallback(() => {
      ...
      for (const principle of models.unfiled) {
        ...
        nodes.push({
          ...
          zIndex: expandedId === principle.id ? 1000 : 0,
    ...
    useEffect(() => {
      if (!expandedId || viewMode !== "map") return;
      let measureFrame = 0;
      let readyFrame = 0;
      measureFrame = requestAnimationFrame(() => {
        readyFrame = requestAnimationFrame(() => {
          const flow = currentFolder ? folderFlow.current : mainFlow.current;
          const canvas = document.querySelector(".tjm-fm-flow .react-flow");
          if (!flow || !canvas) return;

          const currentViewport = flow.getViewport();
          const zoom = Math.max(0.01, Number(currentViewport?.zoom) || 1);
          const edgePadding = compact ? 16 : 28;
          const availableHeight = Math.max(280, (canvas.clientHeight - (edgePadding * 2)) / zoom);
          canvas.style.setProperty("--tjm-principles-card-max-height", `${availableHeight}px`);
          const node = flow.getNode(`principle:${expandedId}`);
          const nodeTop = Number(node?.positionAbsolute?.y ?? node?.position?.y);
          const screenTop = Number.isFinite(nodeTop) ? (nodeTop * zoom) + currentViewport.y : edgePadding;
          if (Number.isFinite(nodeTop) && (screenTop < edgePadding || Math.abs(currentViewport.x) > 0.5)) {
            flow.setViewport({
              x: 0,
              y: screenTop < edgePadding ? edgePadding - (nodeTop * zoom) : currentViewport.y,
              zoom,
            }, { duration: 180 });
          }
        });
    ...
    }, [expandedId, openFolderId, viewMode, compact]);

    const saveViewport = useCallback((mapId, viewport) => {
      mutateLayout((next) => {
        next.viewports[mapId] = { ...viewport, x: 0 };
        return next;
      });
    }, [mutateLayout]);
    ...
    async function savePrinciple({ principle, source, folder, number, name, body, references }) {
      ...
      const result = await runMutation('Saving principle…', () => db.rpc('create_conflict_principle', {
        p_plan_id: PRINCIPLES_PLAN_ID, p_reading_id: source?.reading_id || SENTINEL_READING_ID,
                    ...
                    ` : html`
                      <${ReactFlowProvider}>
                        <${FlowCanvas}
                          ...
                          emptyMessage=${currentFolder ? "This folder is empty." : "Use the menu to create a principle or folder."}
                        />
                      </${ReactFlowProvider}>
                    `}
                  </div>
