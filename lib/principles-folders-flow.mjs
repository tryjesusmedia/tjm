...
if (!CONFIG) {
    ...
    return html`
      <div className="tjm-fm-flow">
        <${ReactFlow}
          ...
          ${nodes.length > 8 && html`<${MiniMap} position="top-right" pannable=${true} zoomable=${true}
  ...
  function FolderMindMap({ bridge }) {
    ...
    useEffect(() => {
      ...
      const focusExpandedPrinciple = () => {
        ...
        measureFrame = requestAnimationFrame(() => {
          focusFrame = requestAnimationFrame(() => {
            ...
            // Keep reading text at its chosen CSS size. Fitting a tall expanded
            // card into the canvas used to shrink the whole map until the larger
            // type looked tiny again, especially on phones.
            const currentViewport = flow.getViewport();
            const zoom = Math.max(1, Number(currentViewport?.zoom) || 1);
            const canvas = document.querySelector(".tjm-fm-flow .react-flow");
            const nodeWidth = node.measured?.width || node.width || (compact ? 348 : 455);
            ...
            const centeredLeft = canvas ? (canvas.clientWidth - (nodeWidth * zoom)) / 2 : edgePadding;
            const left = Math.max(edgePadding, centeredLeft);
            flow.setViewport({
              x: left - (node.position.x * zoom),
              y: edgePadding - (node.position.y * zoom),
              zoom,
            }, { duration: 180 });
          });
        });
      ...
      focusExpandedPrinciple();
      window.addEventListener("tjm-principles-text-size-change", focusExpandedPrinciple);
      return () => {
        window.removeEventListener("tjm-principles-text-size-change", focusExpandedPrinciple);
        cancelAnimationFrame(measureFrame);
        cancelAnimationFrame(focusFrame);
      };
    }, [expandedId, openFolderId, viewMode, compact, mainNodes.length, folderNodes.length]);
