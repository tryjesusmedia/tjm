(() => {
  ...
  function createController(options) {
    ...
    function principleDetail(principle) {
      ...
      return `<article class="principle-detail-card" id="principle-${principle.principle_number}">
    ...
    function groupWindow(group) {
      ...
      return `<article class="principle-group-window ${open ? "is-open" : ""}" data-principle-group-window="${group.key}">
        <button class="principle-group-summary" type="button" data-principle-group="${group.key}" aria-expanded="${open}">
          <span class="principle-first-line">${escapeHTML(firstLine(lowest.body))}</span>
    ...
    function renderTab() {
      ...
      return `<section aria-labelledby="principles-heading" class="principles-view"><header class="view-heading principles-view-heading"><div><p class="eyebrow">YOUR DISCOVERIES</p><h2 id="principles-heading">Principles</h2><p>Each window begins with the first line of its lowest-numbered principle. Open a window to read, edit, revisit, or reorganize the principles inside it.</p></div><div class="principle-spreadsheet-actions"><button class="button button-secondary" type="button" data-export-principles ${principles().length ? "" : "disabled"}>Export spreadsheet</button><button class="button button-primary" type="button" data-import-principles ${principles().length ? "" : "disabled"}>Upload edited spreadsheet</button><input class="principle-file-input" type="file" data-principle-file accept=".xlsx,.xls" hidden></div></header>
        <p class="spreadsheet-help">The spreadsheet contains principle numbers in column 1 and principles in column 2. Edit it in Excel, Google Sheets, Numbers, or another spreadsheet app, save it as an Excel workbook, then upload it here.</p>
    ...
    function openReference(number) {
      ...
      window.setTimeout(() => document.getElementById(`principle-${number}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
    }
