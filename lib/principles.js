  ...
  function createController(options) {
    ...
    async function updateFromForm(form) {
      ...
      try {
        ...
        const existing = principles().find((principle) => principle.id === id);
        const crossReferenceInput = form.querySelector('[name="cross-references"]');
        const crossReferences = crossReferenceInput
          ? parseCrossReferences(crossReferenceInput.value)
          : [...(existing?.cross_reference_numbers ?? [])];
