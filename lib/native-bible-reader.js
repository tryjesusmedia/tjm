(() => {
  ...
  function readerToolbar(translation) {
    return `<div class="nbr-toolbar">
      ...
      <p>Select any words, then choose a highlight color.</p>
    </div>`;
  }
  ...
  async function openPassage(mount, passage, options = {}) {
    ...
    mount.innerHTML = `<div class="nbr-loading" role="status">Opening ${escapeHTML(context.passage)}…</div>`;
    try {
      ...
      mount.innerHTML = `<div class="nbr-error"><strong>The passage could not be opened.</strong><p>${escapeHTML(error.message)}</p><button type="button" data-retry-bible>Try again</button></div>`;
    }
  }
  ...
  async function rerenderReaders(filter = null) {
    ...
    for (const [id, context] of contexts) {
      ...
      if (!filter || filter(context)) jobs.push(openPassage(context.mount, context.passage, context));
    }
  ...
  function showPalette(rect) {
    ...
    const top = Math.max(10, Math.min(window.innerHeight - 70, rect.bottom + 10));
  ...
  function addHighlight(color) {
    ...
    hidePalette();
    saveRemote(item);
    rerenderReaders((context) => context.passage && parsePassage(context.passage).some((spec) => spec.chapterLabel === item.chapterLabel));
  ...
  function ensureUi() {
    ...
    host.innerHTML = `
      <div class="nbr-highlight-palette" id="nbr-highlight-palette" role="toolbar" aria-label="Choose highlight color" hidden>
        ${COLORS.map((color) => `<button type="button" data-choose-highlight-color="${color}" class="nbr-highlight-${color}" aria-label="${COLOR_LABELS[color]}"></button>`).join("")}
      </div>
