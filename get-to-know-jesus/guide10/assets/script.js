(() => {
  ...
  const storageKey = "tjm-jesus-guide-10-progress";
  ...
  const randomVideoCard = document.querySelector("[data-random-youtube]");
  if (randomVideoCard) {
    const channelVideos = [
      { id: "FaUc5fuQN9E", title: "Why Success Can Still Leave You Empty", url: "https://youtu.be/FaUc5fuQN9E" },
      { id: "WYJ9hRn5vNE", title: "The Hedge of Protection", url: "https://youtu.be/WYJ9hRn5vNE" },
      { id: "ynbjuWdPDb0", title: "Can the Bible Really Predict the Future?", url: "https://youtu.be/ynbjuWdPDb0" }
    ];
    const selectedVideo = channelVideos[Math.floor(Math.random() * channelVideos.length)];
    const thumbnail = randomVideoCard.querySelector("[data-video-thumbnail]");
    const videoTitle = randomVideoCard.querySelector("[data-video-title]");
    const videoLink = randomVideoCard.querySelector("[data-video-link]");
    if (thumbnail) {
      thumbnail.src = `https://i.ytimg.com/vi/${selectedVideo.id}/hqdefault.jpg`;
      thumbnail.alt = selectedVideo.title;
    }
    if (videoTitle) videoTitle.textContent = selectedVideo.title;
    if (videoLink) {
      videoLink.href = selectedVideo.url;
      videoLink.setAttribute("aria-label", `Watch ${selectedVideo.title} on YouTube`);
    }
  }
