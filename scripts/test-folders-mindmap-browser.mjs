...
try {
  await page.goto(`http://127.0.0.1:${address.port}/scripts/folders-mindmap-smoke.html`, {
  ...
  const unfiled = page.locator('[data-fm-principle-id="p3"]');
  await unfiled.locator(".tjm-fm-node-menu").click();
  assert.deepEqual(
    await page.locator(".tjm-fm-context-menu button").allTextContents(),
    ["Edit", "Add to Folder", "Delete"],
  );
  ...
  // Dragging one unfiled principle onto another must not create a folder.
  const sourceHandle = unfiled.locator(".fm-node-drag-handle");
  ...
  assert.equal(
    await page.evaluate(() => window.__rpcCalls.filter((call) => call.name === "move_conflict_principles").length),
    0,
  );
  assert.equal(await page.evaluate(() => window.__wentToReading), undefined);
  assert.deepEqual(errors, []);

  // Open a folder and verify the in-folder principle behavior.
  ...
  await filed.locator(".tjm-fm-principle-preview").click();
  await page.waitForFunction(() => document.querySelector('[data-fm-principle-id="p1"]')?.classList.contains("is-expanded"));
  assert.equal(await filed.locator(".tjm-fm-principle-bottom button", { hasText: "Edit" }).count(), 0);
  assert.equal(await filed.locator(".tjm-fm-principle-bottom button", { hasText: "standalone" }).count(), 0);

  await filed.locator(".tjm-fm-node-menu").click();
  assert.deepEqual(
    await page.locator(".tjm-fm-context-menu button").allTextContents(),
    ["Edit", "Remove from Folder", "Delete"],
  );
  ...
  const toggle = page.locator(".tjm-fm-persistent-toggle");
  await toggle.click();
  await page.waitForSelector(".tjm-fm-window", { state: "detached" });
  assert.equal((await toggle.textContent())?.trim(), "Open Mind Map");
