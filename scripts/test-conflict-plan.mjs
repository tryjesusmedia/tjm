...
assert.match(app, /start\.kind === "alias" \? lastAliasTapAt : lastNameTapAt/);
assert.match(app, /now - lastTapAt <= 500/);
...
assert.match(app, /data-toggle-leaderboard/);
assert.match(app, /aria-expanded="\$\{leaderboardOpen\}"/);
assert.doesNotMatch(app, /data-reroll-alias|>Change alias</i);
