...
assert.match(source.module, /screenTop < edgePadding/);
assert.match(source.module, /flow\.setViewport\(\{/);
assert.match(source.module, /zIndex: expandedId === principle\.id \? 1000 : 0/);
...
assert.match(source.module, /data-horizontal-pan="locked"/);
assert.match(source.module, /bottommostNodeTop/);
assert.match(source.module, /1_000_000/);
assert.match(source.module, /next\.viewports\[mapId\] = \{ \.\.\.viewport, x: 0 \}/);
assert.doesNotMatch(source.module, /className="tjm-fm-toolbar-add"/);
assert.doesNotMatch(source.module, /className="tjm-fm-toolbar-search"/);
assert.match(source.module, /next\.mapOpen = false;[\s\S]*goToReadingById/);
...
for (const config of [source.conflictConfig, source.chronConfig]) {
  assert.match(config, /principles-folders-flow\.css\?v=20260911-1/);
...
assert.match(source.css, /react-flow__node:has\(\.tjm-fm-principle\.is-expanded\)[\s\S]*z-index:\s*1000/);
