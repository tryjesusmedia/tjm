...
const base = new URL('../chronbible/', import.meta.url);
...
const window={TJM_CHRONBIBLE_CONFIG:{},matchMedia:()=>({matches:reducedMotion}),addEventListener(){},location:{hash:''},scrollTo(){}};
...
assert.equal(art.catalog.length,313);
assert.equal(new Set(art.catalog.map(b=>[b.motif,b.palette,b.style,b.detail].join(':'))).size,313);
...
assert.equal(get('expanded-reading-badge').alt, art.catalog[4].label);
...
const instrumented=source.replace('  init();',`  window.badgeTest = {
 configure(value){plan=value;activeSection=plan.sections[0].title;session={user:{id:'test-a',user_metadata:{}}};},
 setCompleted(values){completed=new Set(values);},
...
for(const [i,reading] of app.readings().entries()){
 const tasks=reading.bibleTasks;
...
const all=app.readings().flatMap(r=>r.bibleTasks.map(t=>t.progressIndex));app.setCompleted(all);
assert.equal((app.renderEarnedBadges().match(/data-reading-badge=/g)||[]).length,313,'Restored account gets all already-completed badges');
...
console.log('313 chronological badge designs, partial/full/restored progress, reading-title and Progress placement, tap-to-shrink, Escape, reduced motion, focus return, and sign-out passed.');
