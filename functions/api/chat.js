...
function isAllowedWelcomeRequest(request) {
  ...
  try {
    ...
    const isProductionHost = host === "tryjesusmedia.com" || host === "www.tryjesusmedia.com";
    const isCloudflarePreview = host.endsWith(".pages.dev");
    const isLocalPreview = host === "localhost" || host === "127.0.0.1";
    return isWelcomePath && (isProductionHost || isCloudflarePreview || isLocalPreview);
  } catch {
    return false;
  }
}
...
export async function onRequestPost(context) {
  try {
    ...
    if (!isAllowedWelcomeRequest(request)) {
      return json({ error: "Pastor Kal is available only on the Try Jesus Media welcome page." }, 403);
    }

    if (!env.OPENAI_API_KEY) {
      return json({ error: "Pastor Kal is installed, but the OpenAI API key has not been connected yet." }, 503);
    }
